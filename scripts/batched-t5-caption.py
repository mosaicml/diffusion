...
def parse_args():
    ...
    parser.add_argument('--remote_src_base', type=str, required=True, help='Remote base to download MDS-formatted shards.')
    parser.add_argument('--remote_dst_base', type=str, required=True, help='Remote base to write MDS-formatted shards.')
    parser.add_argument('--subdir_paths', nargs='+', type=str, required=True, help='Path to the subdirectory to process.')
...
def main():
    ...
    for subdir_path in tqdm(args.subdir_paths):
        ...
        dataset = StreamingDataset(remote=remote_src, 
                                ...
                                #    predownload=64,
                                   shuffle=False)
        print('StreamingDataset created')
        ...
        writer = MDSWriter(out=output_dir, columns=columns, compression='zstd', hashes=[], size_limit='1GB') # , exist_ok=True)
        ...
        with torch.no_grad():
            for sample_id in tqdm(range(start_idx, end_idx, args.batch_size)):
                ...
                for caption_key in args.caption_keys:
                    ...
                    # TODO double check this grabs the right thing!
                    for i in range(len(samples)):
                        new_sample = samples[i]
                        new_sample[f'{caption_key}_T5_ATTENTION_MASK'] = t5_tokenizer_out['attention_mask'][i].to(
                            torch.bool).numpy().tobytes()
                        new_sample[f'{caption_key}_T5_LATENTS'] = t5_out[0][i].cpu().numpy().tobytes()
                        new_sample[f'{caption_key}_CLIP_ATTENTION_MASK'] = clip_tokenizer_out['attention_mask'][i].to(
                            torch.bool).numpy().tobytes()
                        new_sample[f'{caption_key}_CLIP_LATENTS'] = clip_out.hidden_states[-2][i].cpu().numpy().tobytes()
                        new_sample[f'{caption_key}_CLIP_POOLED_TEXT'] = clip_out[1][i].cpu().numpy().tobytes()
