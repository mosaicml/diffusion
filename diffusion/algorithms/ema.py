...
def compute_ema(model: torch.nn.Module,
    ...
    r"""EMA update function.

    Updates the weights of ``ema_model`` to be closer to the weights of ``model``
...
def get_model_context_manager(model: torch.nn.Module):
    ...
    if fsdp_enabled:
        model_context_manager = model.unet.summon_full_params(model.unet)  # type: ignore
