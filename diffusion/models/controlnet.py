...
class ControlNet(ComposerModel):
    def __init__(self,
                unet,
                vae,
                text_encoder,
                ...
                controlnet_images_key: str = 'controlnet_images',
                precomputed_latents: bool = False,
                encode_latents_in_fp16: bool = False,
                mask_pad_tokens: bool = False,
                fsdp: bool = False,
                sdxl: bool = False,
                load_controlnet_from_composer: bool = False):
        super().__init__()
        self.unet = unet
        self.vae = vae
        self.load_controlnet_from_composer = load_controlnet_from_composer
        ...
        self.controlnet_images_key = controlnet_images_key
        ...
        self.control_image_processor = VaeImageProcessor(
            vae_scale_factor=self.downsample_factor, do_convert_rgb=True, do_normalize=False
        )
    ...
    def _apply(self, fn):
        super(ControlNet, self)._apply(fn)
    ...
    def forward(self, batch):
        ...
        with c:
            # Encode the images to the latent space.
            if self.encode_latents_in_fp16:
                controlnet_image = batch[self.controlnet_images_key].half()
            else:
                controlnet_image = batch[self.controlnet_images_key]

            down_block_sample, mid_block_sample = self.controlnet(
                noised_latents,
                timesteps,
                text_embeds,
                controlnet_image,
        ...
        return self.unet(noised_latents,
                         ...
                         down_block_additional_residuals = down_block_sample,
    ...
    def generate(
        self,
        image: Union[PIL.Image.Image, torch.Tensor, List[PIL.Image.Image], List[torch.Tensor]] = None,
        ...
        control_guidance_start: Optional[float] = 0.0,
        ...
        Args:
            ...
            image (PIL.Image.Image, torch.Tensor, List[PIL.Image.Image], List[torch.Tensor): the images
        ...
        guess_mode = guess_mode or self.controlnet.config.global_pool_conditions
        ...
        image = self._prepare_image(image, width, height, batch_size, num_images_per_prompt, device, self.controlnet.dtype, do_classifier_free_guidance, guess_mode)
        ...
        for i in range(num_inference_steps):
            use_controlnet.append(1.0 - float(i / num_inference_steps < control_guidance_start or (i + 1) / num_inference_steps > control_guidance_end))
        index = 0
        ...
        for t in tqdm(self.inference_scheduler.timesteps, disable=not progress_bar):
            ...
            if guess_mode and self.do_classifier_free_guidance:
                ...
                controlnet_added_cond_kwargs = {
                    "text_embeds": pooled_embeddings.chunk(2)[1],
            ...
            cond_scale = controlnet_conditioning_scale*use_controlnet[index]
